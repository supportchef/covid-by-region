...
const addDataToSpecificRegion = (allTimeseries, key, dataRow, day) => {
  ...
  const rowFound = allTimeseries[key].series.findIndex((row) => row.day === day)
  if (rowFound === -1) {
...
const writeTsFile = (key, depth = 0) => {
  ...
  fs.writeFileSync(`${timeseriesDataLocation}${fileName}.json`, JSON.stringify(filteredObjs));
}
...
const sortAllDates = () => {
  ...
  Object.keys(allTimeseries).forEach(key => {
    ...
    const sortedWithDates = series.map(row => {
